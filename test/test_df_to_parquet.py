...
@mock_s3
@patch('src.df_to_parquet.dataframe_to_parquet',side_effect=Exception('Writing of parquet file to landing bucket failed'))
def test_that_exception_thrown_for_invalid_arguments(
    patched_dataframe_to_parquet
):
    with pytest.raises(Exception, match=r'Writing of parquet'):
        ...
        patched_dataframe_to_parquet(df,bucket,key)
